...
class EmailFilterTasks:
		...
		return Task(
			description=dedent(f"""\
			  ...
			  Use your expertise in email content analysis to distinguish
				...
				Your final answer MUST be a list of the relevant thread_ids and the sender, use bullet points.
				"""),
			expected_output="A list of relevant thread_ids and the sender",
			agent=agent
		)
	...
	def action_required_emails_task(self, agent):
		return Task(
			description=dedent("""\
				...
				"""),
			expected_output="A list of action-required emails with the required information",
			agent=agent
		)
	...
	def draft_responses_task(self, agent):
		return Task(
			description=dedent(f"""\
				Based on the action-required emails identified, draft responses for each based on the documentation.
				...
				- ALWAYS research about it in the documentation.
				- DO a research BEFORE drafting the response.
				...
				"""),
			expected_output="A confirmation that all responses have been drafted",
			agent=agent
		)