...
class EmailFilterAgents():
	...
	def email_response_writer(self):
		return Agent(
			...
			goal='Draft responses to action-required emails based on documentation and context',
			backstory=dedent("""\
				...
				tailored to address the specific needs and context of the email. You should give answers based
			    only in the context offered by the documents sourced by the DirectorySearchTool and PDFSearchTool tools."""),
			tools=[
				DirectorySearchTool(directory='/Users/gabrielrenno/Documents/ai_crew/auto_email/source_files'),
				PDFSearchTool(),
				GmailGetThread(api_resource=self.gmail.api_resource),
